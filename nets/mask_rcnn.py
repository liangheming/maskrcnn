...
class MaskHead(nn.Module):
    def __init__(self, in_channel, inner_channels, reduce_dim, num_cls):
        super(MaskHead, self).__init__()
        conv_list = list()
        next_channel = in_channel
        for inner_channel in inner_channels:
            conv_list.append(nn.Conv2d(next_channel, inner_channel, kernel_size=3, stride=1, padding=1))
            # conv_list.append(nn.BatchNorm2d(inner_channel))
            conv_list.append(nn.ReLU(inplace=True))
            next_channel = inner_channel
        conv_list.append(nn.ConvTranspose2d(next_channel, reduce_dim, 2, 2, 0))
        conv_list.append(nn.ReLU(inplace=True))
        conv_list.append(nn.Conv2d(reduce_dim, num_cls, 1, 1, 0))
...
class ROIHead(nn.Module):
    ...
    def forward(self, xs, proposals, valid_size, targets=None):
        ...
        positive_gt_idx = None
        predicts = None
        loss = dict()
        if self.training:
            cls_loss, box_loss, positive_ids, positive_gt_idx = self.compute_box_loss(proposals,
                                                                                      cls_predicts,
                                                                                      box_predicts,
                                                                                      targets)
            loss['roi_cls_loss'] = cls_loss
            loss['roi_box_loss'] = box_loss
            box_predicts = [b[p].detach().clamp(min=0) for p, b in zip(positive_ids, box_predicts)]
            positive_gt_idx = positive_gt_idx
        else:
            predicts = self.post_process(cls_predicts, box_predicts, valid_size)
            box_predicts = [p[0] for p in predicts]
        nums_per_batch = [len(b) for b in box_predicts]
        mask_feature = self.mask_pooling(xs, box_predicts, hw_size)
        mask_predict = self.mask_head(mask_feature)
        mask_predict = mask_predict.split(nums_per_batch)
        ret = None
        if self.training:
            mask_loss = self.compute_mask_loss(mask_predict, positive_gt_idx, box_predicts, targets)
            loss['roi_mask_loss'] = mask_loss
        else:
            ret = self.inference_mask(mask_predict, predicts)
        return ret, loss
    ...
    def post_process(self, cls_predicts, box_predicts, valid_size):
        ...
        for cls, box, wh in zip(cls_predicts, box_predicts, valid_size):
            ...
            boxes, scores, labels = boxes[keep], scores[keep], labels[keep]
            predicts.append((boxes, labels, scores))
        return predicts

    @torch.no_grad()
    def inference_mask(self, mask_predict, box_predicts):
        ret = list()
        for mp, (b, l, s) in zip(mask_predict, box_predicts):
            if len(mp):
                mp = mp[range(len(mp)), l.long()].sigmoid()
            else:
                mp = mp[:, 0, :, :]
            pred = torch.cat([b, s[:, None], l[:, None]], dim=-1)
            ret.append((pred, mp))
        return ret
...
class MaskRCNN(nn.Module):
    def __init__(self, **kwargs):
        ...
        box_in_channel = self.cfg['fpn_channel'] * roi_pooling.output_size[0] ** 2
        box_head = FasterRCNNSimpleBoxHead(in_channels=box_in_channel,
                                           inner_channels=self.cfg['roi']['fc_out_channels'],
                                           num_cls=self.cfg['num_cls'])
        mask_roi_pooling = MultiScaleRoIAlign(
            featmap_names=self.cfg['mask']['featmap_names'],
            output_size=self.cfg['mask']['output_size'],
            sampling_ratio=self.cfg['mask']['sampling_ratio']
        )
        mask_head = MaskHead(
            self.cfg['fpn_channel'],
            inner_channels=self.cfg['mask']['mask_layers'],
            reduce_dim=self.cfg['mask']['mask_dim_reduce'],
            num_cls=self.cfg['num_cls']
        )
        self.roi_head = ROIHead(box_head=box_head,
                                roi_pooling=roi_pooling,
                                mask_head=mask_head,
                                mask_pooling=mask_roi_pooling,
                                pos_iou_thr=self.cfg['roi']['pos_iou_thr'],
                                neg_iou_thr=self.cfg['roi']['neg_iou_thr'],
                                allow_low_quality_matches=self.cfg['roi']['allow_low_quality_matches'],
                                add_gt_as_proposals=self.cfg['roi']['add_gt_as_proposals'],
                                sample_size=self.cfg['roi']['sample_size'],
                                positive_fraction=self.cfg['roi']['positive_fraction'],
                                bbox_weights=self.cfg['roi']['bbox_weights'],
                                iou_type=self.cfg['roi']['iou_type'],
    ...
    def forward(self, x, valid_size, targets=None):
        ...
        for k, v in zip(self.feature_keys, xs):
            feature_dict[k] = v
        predicts, roi_losses = self.roi_head(feature_dict, boxes, valid_size, targets)
        losses = {}
